...
autocmd BufRead,BufNewFile *rc set syntax=sh
...
if has("autocmd")
  augroup templates
    autocmd BufNewFile *.py 0r ~/.vim/skel.py | %s/skel/\=expand('%:r')/g
    autocmd BufNewFile *.slurm 0r ~/.vim/skel.slurm | %s/skel/\=expand('%:r')/g
  augroup END
endif
